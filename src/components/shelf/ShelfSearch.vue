<template>
  <div class="shelf-search-wrapper" :class="{'search-top': ifInputClicked}">
    <!-- 如果点击了input框的话就让我们的整个搜索框的top值改为0实现上移 -->
    <div class="shelf-search" :class="{'search-top': ifInputClicked}">
      ...
      <!-- 取消按钮 点击输入框后出现取消 默认不显示(与中英文图标二选一出现)-->
...
export default {
  ...
  data() {
    return {
      searchText: "", // input框的内容
      ifInputClicked: false, //是否点击了input框
      selectedTab: 1, // 头部导航栏选中的项 
    };
  },
  computed: {
    lang() {
      // 根据插件 i18n 的 locale属性显示
